...
public class Ball {
    ...
    */
   public void moveOneStepWithCollisionDetection(ContainerBox box,Board board,Brick brick) {
      ...
      if ( y + radius/2 >= y + 150 - board.board_size_y ){ 
			if ( x >=  board.board_x &&  board.board_x <=  board.board_x +  board.board_size_x ){
				speedY = -speedY;
				speedX = ( x - (  board.board_x + board.board_size_x/2 ) ) / 3;
			}
			
			for ( int i = 0 ; i < 8 ; i++ )
			{
				if (  x >= brick.brick_x[i] && x <= brick.brick_x[i] + brick.brick_size_x[i] ){
					if ( y - radius/2 <= brick.brick_y[i] + brick.brick_size_y[i] && y + radius/2 >= brick.brick_y[i] ){
						if ( speedY < 0 ) {
							speedY = -speedY;
							brick.brick_x[i] = 0;
							brick.brick_y[i] = 0;
							brick.brick_size_x[i] = 0;
							brick.brick_size_y[i] = 0;
						}
